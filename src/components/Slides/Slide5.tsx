...
import { Sliders, Play, Pause, RotateCcw, TrendingDown, BarChart3, ZoomIn, Layers, Zap, Target } from 'lucide-react'
...
const skewedValley = (x: number, y: number): number => {
  return 2*x**2 + y**2 - x*y - 3*x - y + 4
}

// Gradient of Skewed Valley
const gradSkewedValley = (x: number, y: number): [number, number] => {
  const dx = 4*x - y - 3
  const dy = 2*y - x - 1
  return [dx, dy]
}
...
export default function Slide5() {
  ...
  const generatePath = useMemo(() => {
    ...
    pathX.push(x)
    pathY.push(y)
    const initialLoss = skewedValley(x, y)
    ...
    for (let t = 1; t <= MAX_ITERATIONS; t++) {
      const [dx, dy] = gradSkewedValley(x, y)
  ...
  const currentLoss = skewedValley(currentX, currentY)
  const [gradX, gradY] = gradSkewedValley(currentX, currentY)
  ...
  const globalMinZ = skewedValley(GLOBAL_MINIMUM[0], GLOBAL_MINIMUM[1])
  const startLoss = skewedValley(params.startX, params.startY)

  // Prepare 3D plot data
  const plot3DData = [
    // Skewed Valley surface
    {
      type: 'surface',
      x: surfaceData.x,
      y: surfaceData.y,
      ...
      name: 'Skewed Valley Surface',
      hoverinfo: 'x+y+z+name',
      showlegend: true
    },
    // Adam path - UPDATES LIVE
    {
      type: 'scatter3d',
      mode: 'lines+markers',
      name: 'Adam Path',
  ...
  const plot3DLayout = {
    title: {
      text: `Adam on Skewed Valley • Iteration: ${iteration}/${MAX_ITERATIONS}${isAnimating ? ' • LIVE' : ''}`,
  ...
  return (
    <div id="slide-5" className="min-h-screen py-12 px-4">
      <div className="container mx-auto">
        <div className="text-center mb-8">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-pink-600/20 to-rose-600/20 px-4 py-2 rounded-full mb-3">
                ...
                <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800 rounded-xl p-4 h-full">
                  ...
                  <div className="space-y-3">
                    <div className="p-3 bg-gray-800/30 rounded-lg">
                      <div className="text-xs text-gray-400 mb-1">Current Position (x,y)</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="p-3 bg-blue-900/20 rounded-lg">
                  ...
                  <p className="text-sm text-gray-300">
                    Click "Start Animation" to watch Adam navigate the Skewed Valley step by step.
                  </p>
                </div>
